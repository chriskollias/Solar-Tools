...
class NREL_API():
    ...
    def get_monthly_averages(self, lat_input, long_input, year_input, attributes_input):
        ...
        solar_data_csv = self.retrieve_data_from_api(lat, lon, year, interval, leap_year, attributes)
        df = pd.read_csv(solar_data_csv)

        graph_image = self.display_csv_graph(df)

        return graph_image

    def retrieve_data_from_api(self, lat, lon, year, interval, leap_year, attributes):
        # Declare url string
